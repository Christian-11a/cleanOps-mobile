...
export default function ReviewsScreen() {
  ...
  const renderItem = ({ item }: { item: any }) => {
    const isReceived = item.reviewee_id === user?.id;
    const name = isReceived ? item.reviewer?.full_name : item.reviewee?.full_name;
...
const st = StyleSheet.create({
  ...
  type: { fontSize: 11, color: '#64748b', marginTop: 2 },
